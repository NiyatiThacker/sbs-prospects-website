...
async function testFetch() {
  const { data: raw } = await supabase.from('screentime_raw_logs')
    .select('timestamp')
    .eq('employee_id', '23067bbf-cb27-4c5d-9b86-8ec9efc0c62f')
    .gte('timestamp', '2026-09-24T18:30:00Z') // midnight IST
    .order('timestamp', { ascending: true })
    .limit(3);
  console.log("Raw logs today:", JSON.stringify(raw, null, 2));
}
testFetch();
