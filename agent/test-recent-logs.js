...
async function testFetch() {
  const { data: raw } = await supabase.from('screentime_raw_logs')
    ...
    .eq('employee_id', '23067bbf-cb27-4c5d-9b86-8ec9efc0c62f') // Niyati's ID, wait, is it niyati testing the break?
    .order('timestamp', { ascending: false })
    .limit(5);
  console.log("Recent raw logs:", JSON.stringify(raw, null, 2));
}
testFetch();
