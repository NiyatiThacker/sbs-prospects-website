...
export default function EmployeeDirectoryPage() {
  ...
  const [editForm, setEditForm] = useState({ role: '', department: '', expected_shift_start: '', expected_shift_end: '' });
    ...
    },
    {
      ...
      render: (val, row) => (
        <div style={{ display: 'flex', gap: '8px', justifyContent: 'flex-end' }}>
          <button onClick={(e) => { e.stopPropagation(); setEditingEmployee(row); setEditForm({ role: row.role, department: row.department, expected_shift_start: row.expected_shift_start || '09:00:00', expected_shift_end: row.expected_shift_end || '18:00:00' }); }} style={actionBtnStyle} title="Edit Employee">
            <Edit size={16} />
          </button>
        ...
        <div style={{
          ...
          <div style={{
            ...
            <form onSubmit={handleEditEmployee} style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
              ...
              <div>
                <label style={{ display: 'block', marginBottom: '6px', fontSize: '13px', color: 'var(--color-text-secondary)' }}>Expected Shift End</label>
                <input required type="time" value={editForm.expected_shift_end ? editForm.expected_shift_end.substring(0, 5) : '18:00'} onChange={e => setEditForm({...editForm, expected_shift_end: e.target.value + ':00'})} style={inputStyle} />
              </div>
              <Button type="submit" disabled={isSubmitting} fullWidth style={{ marginTop: '8px' }}>
